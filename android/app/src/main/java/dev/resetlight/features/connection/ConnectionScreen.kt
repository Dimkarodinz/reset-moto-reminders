...
private fun DtcRow(dtc: DecodedDtc) {
    ...
    Column {
        ...
        Text(
            text = stringResource(dtcProvenanceLabelResource(dtc.message.status)),
...
@StringRes
internal fun dtcProvenanceLabelResource(status: DtcMessageStatus): Int = when (status) {
    DtcMessageStatus.OEM_CONFIRMED -> R.string.dtc_source_oem_confirmed
    DtcMessageStatus.VEHICLE_OBSERVED -> R.string.dtc_source_vehicle_observed
    DtcMessageStatus.OPEN_DATA_GENERIC -> R.string.dtc_source_open_data_generic
    DtcMessageStatus.INFERRED -> R.string.dtc_source_inferred
    DtcMessageStatus.GENERIC_CLASSIFICATION,
    DtcMessageStatus.UNKNOWN,
    -> R.string.dtc_source_unverified
}
