...
enum class DtcMessageStatus(val serializedValue: String) {
    ...
    OPEN_DATA_GENERIC("open_data_generic"),
    INFERRED("inferred"),
 ...
 * [messages] by exact entry code, [genericMessages] by base code.
 */
data class DtcTranslation(
    ...
    val genericMessages: Map<String, String>,
    val sourceSha256: String,
)
...
class LocalizedDtcDescriptions(
    ...
    private fun translationFor(normalized: String, english: DtcMessage): String? = when (english.status) {
        DtcMessageStatus.OPEN_DATA_GENERIC -> translation.genericMessages[english.baseCode]
...
data class DtcOpenDataMetadata(
    val sourceName: String,
...
data class DtcDictionary(
    ...
    val genericCatalog: DtcOpenDataMetadata,
    val genericEntries: Map<String, String>,
    val sourceSha256: String,
) : DtcDescriptionLookup {
    ...
    fun mappedDescriptionFor(code: String): DtcMessage? {
        ...
        return entries[normalized]
            ...
            ?: genericEntries[normalized.takeIf { it.length >= 5 }?.take(5)]?.let { message ->
                DtcMessage(
                    message = message,
                    status = DtcMessageStatus.OPEN_DATA_GENERIC,
                    baseCode = normalized.take(5),
                    evidence = DtcMessageEvidence(
                        vehicleObserved = false,
                        sourceKind = "open_data",
                        sourceName = genericCatalog.sourceName,
                        sourceVersion = genericCatalog.sourceRevision,
                        wording = "upstream_cc0",
                        oemConfirmed = false,
                    ),
                )
            }
