...
class ProfileSchemaTest {
    ...
    @Test
    fun `DTC translations contain only codes present in the English generic catalogue`() {
        val root = File("build/generated/profileAssets/profiles")
        val genericKeys = yaml.readTree(
            File(root, "triumph-tiger-900-gt-pro-2021.en.dtcmap.yaml"),
        ).path("generic_entries").fieldNames().asSequence().toSet()

        TRANSLATION_LOCALES.forEach { locale ->
            val translated = yaml.readTree(
                File(root, "triumph-tiger-900-gt-pro-2021.$locale.dtctranslation.yaml"),
            ).path("generic_messages").fieldNames().asSequence().toSet()
            assertTrue("$locale contains unknown generic codes", genericKeys.containsAll(translated))
        }
    }
