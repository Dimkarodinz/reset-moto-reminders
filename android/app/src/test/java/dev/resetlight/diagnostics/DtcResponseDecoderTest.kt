...
import dev.resetlight.profiles.DtcOpenDataMetadata
...
class DtcResponseDecoderTest {
    private val dictionary = DtcDictionary(
        ...
        entries = mapOf(
            "P1577-00" to DtcMessage(
                message = "Observed motorcycle-specific powertrain code.",
                status = DtcMessageStatus.VEHICLE_OBSERVED,
            ),
        ),
        genericCatalog = DtcOpenDataMetadata(
            sourceName = "test-open-data",
            sourceRevision = "revision",
            license = "CC0-1.0",
        ),
        genericEntries = emptyMap(),
    ...
    fun `decodes captured confirmed DTC and dictionary message`() {
        ...
        assertEquals(
            "Observed motorcycle-specific powertrain code.",
            dtc.message.message,
        )
    }
