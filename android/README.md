...
Current build: **v0.6.0**. Reset Moto Reminders connects to a bonded `vLinker MC-Android` (tested with the **vLinker MC+** OBD adapter / ECU linker on Android and iPhone), verifies the captured ELM/STN identity, applies the common adapter initialization sequence and disconnects cleanly.
...
The two write paths are hardware-unvalidated and exist to be exercised in one motorcycle trip (see the harvest sweep below). They cannot appear in a release build.

Checkpoint: v0.4.0 (`versionCode 5`) completed the consolidated motorcycle research test on 2026-08-10 — adapter initialization, identity match, all identifier reads and a default-session zero-DTC count read, with every command inside the read-only allowlist (journal and analysis in `logs/2026-08-10/`, root `AGENTS.md` checkpoint updated). v0.5.0 (`versionCode 6`) added the mainline DTC read and the research instrument read on top of that validated path. v0.6.0 (`versionCode 7`) adds the two gated writes; all four research paths and both writes are covered by transcript/scripted unit tests (121 tests). The DTC detail read, extended-session branch, the instrument path on hardware, DTC clear and service reset remain unexercised against a motorcycle.
...
- **Localization.** Localize the UI into English, Spanish and Ukrainian, chosen automatically from the phone's language setting, with English as the default and fallback. Move on-screen strings into `res/values/strings.xml` with `values-es`/`values-uk` overlays; only UI text is translated — protocol bytes, ECU/DTC map data and journal contents stay untouched. User-facing DTC messages already live in language-tagged maps (`*.en.dtcmap.yaml`), so add `es`/`uk` message maps rather than translating codes in code.
...
- **Miles support.** Make the app represent the bike's own unit: it shows and accepts whatever unit the dashboard uses and never displays or writes a value that disagrees with it. There is deliberately no phone-locale conversion — the bike is the only source of truth. The app currently hardcodes metric. First run the "flip the dash unit, capture twice" experiment (Run 3 of the trip above) to learn whether the cluster stores a canonical unit or the rider's chosen unit. If canonical (display-only toggle), just label the displayed unit to match the dash while the bytes stay unchanged — safe with no further capture. If the cluster stores the chosen unit, read that unit from the cluster, label reads in it and accept/write interval values in the same unit, never assuming it and refusing the write when it cannot be established, with the unit folded into the fingerprint gate. Do not claim miles support until a miles capture confirms the behavior.
