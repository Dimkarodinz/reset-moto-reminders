...
- Main Android **v0.7.0** (`versionCode 11`) and Triumph Research **v0.4.0** (`versionCode 4`) pass their unit tests, lint and debug builds and are installed on the Samsung SM-A202F. The main app hides profile-gate wording before connection; service reset is a normal supported-profile action and only DTC clear is labelled Beta. Both use the fixed dark theme plus a bold italic action-style title.
