...
- **Experimental instrument-family reset update (2026-09-07):** the Android family catalogue now exposes service reset for the updated-generation Tiger 900 and Tiger Sport 660 as experimental. Both paths use the shared 29-bit instrument route and timing-selected 16-byte-seed SecurityAccess, but retain separate payload builders: the updated TFT preserves live `A500`/`A000` fields and writes the absolute next-service odometer, while the hybrid display uses its 25 km representation. Each sends one explicit ISO-TP write sequence, requires its positive response, reads `A000`/`A010` back and treats any failure after the first write frame as ambiguous without retrying. The validated E64 original-TFT path is unchanged. Other engine-only motorcycle groups remain DTC-only.
