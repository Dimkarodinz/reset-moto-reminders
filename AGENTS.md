...
2. iOS using Swift, SwiftUI and CoreBluetooth. On-device installation, launch and the primary adapter identity path are validated; motorcycle operations are pending a v0.1.3 retest.
...
| iOS application | [`ios/README.md`](ios/README.md), [`ios/AGENTS.md`](ios/AGENTS.md), [`ios/PLAN.md`](ios/PLAN.md), [`ios/VLINK_CONNECTION.md`](ios/VLINK_CONNECTION.md) | Native app/build guide, tested core, CoreBluetooth transport, observed primary channel and pending motorcycle retest |
...
- Native iOS preview **v0.1.3** (`build 4`) is implemented in [`ios/ResetMotoReminders/`](ios/ResetMotoReminders/) with its platform-neutral TDD core in [`ios/ResetMotoCore/`](ios/ResetMotoCore/). All **35** protocol/use-case tests plus **2** presentation-contract tests pass; unsigned simulator/generic-device and signed physical-device builds pass. The update is installed and launch-verified on the iPhone 12.
- The first powered iOS run reached **Motorcycle connected**, proving the production app's primary `18F0` notification/write path and accepted `ATI` identity. All features then stopped at `ATWS`: iOS expected bare `OK`, while the adapter correctly returned an `ELM327` identity banner. v0.1.3 fixes and regression-tests this command-specific response without weakening validation of ordinary setup commands. Motorcycle reads and writes remain pending retest, not a resolved compatibility claim.
...
- Current physical-test boundary: the km reset is project-app validated; the miles reset is capture-validated and implemented but still needs one project-app run; nonzero DTC detail and DTC clear remain hardware-unvalidated. Write actions remain debug-only until the release gates below are complete.
...
| MC-IOS transport | Advertisement and complete GATT layout captured; the production app completed `ATI` over `18F0`/`2AF1` with replies on `2AF0` | Exact identity/transcript was not retained; initialization and motorcycle operations need a v0.1.3 retest |
...
5. ~~Implement DTC clear and service reset against fake and replay transports~~ — done. v0.7.0 adds the captured miles path and correct single-send DTC pending-response handling. Live writes remain debug-only behind their gates. Remaining hardware steps: one miles reset through the project app, then read + clear a controlled ABS-sensor fault.
6. Progress each live Android operation independently as its prerequisites and identity checks become validated.
7. ~~Perform the MC-IOS `ATI` proof and begin the native iOS app~~ — primary production channel confirmed. Next run the corrected dashboard and DTC reads on the motorcycle before testing intentional writes.
...
11. ~~Support motorcycles set to miles~~ — **implemented in v0.7.0 from successful 2026-08-22 captures.** Reads remain canonical kilometres. Interval writes are unit-dependent: `0x33` carries hundreds of kilometres and `0x34` carries hundreds of miles. Because no reliable read reports the dashboard unit, the app asks the rider to select the unit currently shown and does not use phone locale. Remaining gate: validate one miles reset through the project app before claiming project-app miles support or enabling writes in a release build.
