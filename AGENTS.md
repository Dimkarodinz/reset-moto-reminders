...
- Main Android **v0.11.0** (`versionCode 15`), Triumph Research **v0.4.0** (`versionCode 4`) and General Motorcycle Research **v0.2.0** (`versionCode 2`) pass their respective unit tests, lint and builds. The main app packages the 10-entry Triumph catalogue and separate OBDLink CX/MX/LX/MX+ profiles while retaining the validated vLinker/Tiger 900 path. It hides profile-gate wording before connection; service reset is a normal selected-profile action and only DTC clear is labelled Beta. General Research remains a finite read-only standard-OBD collector and must not inherit Triumph write commands.
- Native iOS **v0.3.0** (`build 8`) is implemented in [`ios/ResetMotoReminders/`](ios/ResetMotoReminders/) with its platform-neutral TDD core in [`ios/ResetMotoCore/`](ios/ResetMotoCore/). It loads the shared 10-entry Triumph catalogue, supports original, updated, hybrid and live-detected combined service formats, and selects the matching DTC-clear strategy. It has English-default German, Spanish, French and Ukrainian localization. **50** protocol/use-case tests plus **6** presentation-contract tests pass. The corrected connection and dashboard/odometer read are physically validated on an iPhone 12; the added profiles and combined-write modes are not.
