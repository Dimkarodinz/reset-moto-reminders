...
The validated target is a 2021 Triumph Tiger 900 GT Pro connected through a vLinker MC+. Android v0.12.0 and iOS v0.4.0 expose the same family-composed motorcycle choices; Android additionally contains exact Bluetooth Classic profiles for OBDLink LX, MX and MX+. Bounded experimental support is described in [`ecu-maps/README.md`](ecu-maps/README.md). Do not present those choices as validated until a physical adapter/motorcycle transcript is retained. Do not expand the product into ECU coding or calibration, firmware flashing, actuator tests, arbitrary diagnostic scanning, emissions changes, immobilizer work or security-access brute forcing.
...
| DTC dictionary | [`dtc-maps/triumph-tiger-900-gt-pro-2021.en.dtcmap.yaml`](dtc-maps/triumph-tiger-900-gt-pro-2021.en.dtcmap.yaml), [`dtc-maps/dtcmap.schema.json`](dtc-maps/dtcmap.schema.json) | One observed Tiger code plus a pinned CC0 generic OBD-II fallback; unmatched manufacturer-specific codes receive a neutral subsystem message |
...
| Experimental Android adapter | [`adapter-maps/obdlink-mx-android.adaptermap.yaml`](adapter-maps/obdlink-mx-android.adaptermap.yaml) | Original OBDLink MX Bluetooth Classic pairing, SPP and identity rules; Android implementation complete, physical project validation pending |
| Experimental Android adapters | [`adapter-maps/obdlink-lx-android.adaptermap.yaml`](adapter-maps/obdlink-lx-android.adaptermap.yaml), [`adapter-maps/obdlink-mx-plus-android.adaptermap.yaml`](adapter-maps/obdlink-mx-plus-android.adaptermap.yaml) | Separate Classic/RFCOMM profiles with exact names and `STDI` product identity; not aliases of original MX |
| Triumph family profiles | [`ecu-maps/README.md`](ecu-maps/README.md), [`ecu-maps/triumph.motorcycleprofiles.yaml`](ecu-maps/triumph.motorcycleprofiles.yaml) | Engine and instrument families compose capability-specific validated or experimental motorcycle profiles |
...
## Current cross-platform checkpoint — 2026-09-07

- **Live-adaptive instrument reset update (2026-09-07):** the Android and iOS experimental catalogues expose service reset for the updated-generation Tiger 900, Tiger Sport 660 and all other listed modern Triumph groups. Known Tiger profiles retain their explicit updated/hybrid association; the broader groups use one adaptive family on the shared 29-bit instrument route. After session/security and read-only precursor requests, exact `A000` data length selects the encoding: 12 bytes uses updated TFT, 5 uses hybrid, and every other shape blocks before `2EA000`. Input step and hybrid 25 km absolute-odometer divisor are separate map fields. The selected encoding is carried through post-write verification, and no alternate encoding is retried. The validated E64 original-TFT path is unchanged. All newly broadened profiles remain experimental and not hardware-validated.

- **Original OBDLink MX Android update (2026-08-31):** Android v0.10.0 (`versionCode 14`) adds a separate experimental Bluetooth Classic profile for the discontinued original OBDLink MX. Selection requires the exact bonded name `OBDLink MX`; `OBDLink MX+` is rejected. The profile uses the standard SPP UUID, physical Connect-button pairing without a guessed PIN, the manufacturer-defined `STDI` hardware identity and the original-MX `STN115` firmware family. RFCOMM construction now receives the selected profile instead of capturing the primary vLinker profile. Existing Tiger commands and all write gates are unchanged. Hardware validation remains pending.

- **OBDLink CX release update (2026-08-31):** Published Android v0.9.0 (`versionCode 13`) and iOS v0.2.0 (`build 7`) add a separate CX BLE profile using only service `FFF0`, notify characteristic `FFF1` and acknowledged writes to `FFF2`. Both platforms require exact advertised name/service/layout plus a recognizable `ATI` identity before motorcycle commands. Writes are serialized in negotiated-size chunks, notifications still complete only at the ELM prompt, and interrupted writes preserve the existing ambiguous/no-retry behavior. CX is labelled experimental in the apps and public pages because no physical CX session has been run by this project. The existing vLinker paths and exact Tiger motorcycle gates are unchanged.
...
- Main Android **v0.12.0** (`versionCode 16`) passes its unit tests, lint and release build. It packages one observed Tiger code and 220 pinned CC0 generic OBD-II titles, plus separate OBDLink CX/MX/LX/MX+ profiles, while retaining the validated vLinker/Tiger 900 path. Each displayed DTC carries a small provenance label so observed, CC0 generic, inferred and unverified wording are never presented as equally authoritative. It hides profile-gate wording before connection; service reset is a normal selected-profile action and only DTC clear is labelled Beta. Private research applications are local-only under the ignored `research-builds/` tree.
- Native iOS **v0.4.0** (`build 9`) is implemented in [`ios/ResetMotoReminders/`](ios/ResetMotoReminders/) with its platform-neutral TDD core in [`ios/ResetMotoCore/`](ios/ResetMotoCore/). It loads the same observed/open-data DTC dictionary and provenance labels, supports original, updated, hybrid and live-detected combined service formats, and selects the matching DTC-clear strategy. It has English-default German, Spanish, French and Ukrainian localization. **50** protocol/use-case tests plus **7** presentation-contract tests pass. The corrected connection and dashboard/odometer read are physically validated on an iPhone 12; the added profiles and combined-write modes are not.
...
- The iOS app mirrors dashboard/odometer read, DTC read, DTC clear (Beta), and km/miles service reset. Motorcycle capabilities and commands, adapter profiles and DTC descriptions are generated into a typed JSON resource from the shared maps by [`ios/tools/sync_profiles.rb`](ios/tools/sync_profiles.rb); feature views contain no protocol bytes.
...
- iOS operations are serialized for the complete multi-command flow. DTC clear sends the clear request once through response-pending and verifies the count. Service reset validates distance/date before traffic. The original Tiger flow requires status `043`; combined flows require an exact supported `A000` shape before writing and verify the selected shape afterward. Malformed replies after a write are reported as requiring inspection, transport interruptions remain ambiguous, and neither path retries automatically.
...
Keep user-facing DTC messages separate from motorcycle command profiles. Within a motorcycle DTC map, keep observed/validated `entries` distinct from CC0 `generic_entries`. Resolve a motorcycle-specific exact/base code first, then a pinned open-data generic code, then a generic subsystem message; malformed values use the neutral invalid-code fallback. Never copy descriptions from a merely public or unlicensed catalogue. Every imported dataset needs an explicit reusable licence and pinned revision. Generic entries do not establish motorcycle compatibility or enable an operation.
...
7. What is Triumph's independently reusable description for captured DTC `0x157700` / inferred display `P1577-00`? Until one is found, the dictionary reports only that this motorcycle-specific code was observed.
