...
| iOS application | [`ios/README.md`](ios/README.md), [`ios/AGENTS.md`](ios/AGENTS.md), [`ios/PLAN.md`](ios/PLAN.md), [`ios/VLINK_CONNECTION.md`](ios/VLINK_CONNECTION.md) | Native app/build guide, tested core, CoreBluetooth transport, GATT layout and pending physical adapter proof |
...
- Native iOS preview **v0.1.2** (`build 3`) is implemented in [`ios/ResetMotoReminders/`](ios/ResetMotoReminders/) with its platform-neutral TDD core in [`ios/ResetMotoCore/`](ios/ResetMotoCore/). All **32** protocol/use-case tests plus **2** presentation-contract tests pass; unsigned simulator/generic-device and signed physical-device builds pass in Xcode 26.6. The app is installed and launch-verified on an iPhone 12.
- The 2026-08-23 critical/high iOS review closed five fail-closed gaps: stale DTC reads can no longer authorize a clear after a failed refresh or prior clear attempt; live diagnostic extraction requires the configured CAN response ID; a command completes only after both CoreBluetooth's `.withResponse` acknowledgement and a prompt-terminated ELM response; callbacks from obsolete central/peripheral instances are ignored; and entering the background closes the session, reporting an interruption after a state-changing write as ambiguous. The unproven MC-IOS primary command channel remains the hardware-validation boundary, not a resolved compatibility claim.
