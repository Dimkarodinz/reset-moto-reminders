...
| Instrument reads | **Project app received live `5E01`/`0D01` responses on 2026-08-12**: status `"043"` matching the validated fingerprint and odometer `0xAE9C` (44700 km), payloads byte-identical to the third-party captures modulo the `704` CAN-ID framing | The app's decode of those live frames is fixed but not yet re-run on hardware; `0D01` semantics (read-only vs capture) remain unconfirmed |
