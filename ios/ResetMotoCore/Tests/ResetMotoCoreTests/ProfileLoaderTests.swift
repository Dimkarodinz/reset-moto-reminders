...
final class ProfileLoaderTests: XCTestCase {
  func testBundledProfilePinsSupportedMotorcycleAndSupportedGattChannels() throws {
    ...
    XCTAssertEqual(3, profile.schemaVersion)
  ...
  func testBundledProfileContainsEverySupportedDtcLanguage() throws {
    ...
    XCTAssertEqual(
      ...
      profile.dtcDescriptions(forLanguage: "en")["P0030"])
    XCTAssertEqual(
      "Lambdasonden-Heizung Schaltung (Bank 1, Sonde 1)",
      profile.dtcDescriptions(forLanguage: "de-DE")["P0030"])
    XCTAssertEqual(.vehicleObserved, profile.dtcDescriptionProvenance["P1577-00"])
    XCTAssertEqual(.openDataGeneric, profile.dtcDescriptionProvenance["P0030"])
    XCTAssertEqual(
