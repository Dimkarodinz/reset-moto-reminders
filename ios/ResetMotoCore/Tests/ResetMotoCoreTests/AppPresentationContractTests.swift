...
struct AppPresentationContractTests {
  ...
  @Test("DTC rows show readable secondary provenance text")
  func dtcProvenance() throws {
    let source = try String(contentsOf: appSource("ContentView.swift"), encoding: .utf8)
    let english = try #require(localizedStrings("en"))

    #expect(source.contains("dtc.descriptionProvenance.localizationKey"))
    #expect(source.contains(".font(.caption2)"))
    #expect(source.contains(".foregroundStyle(.secondary)"))
    #expect(english["dtc_source_open_data_generic"] == "Generic OBD-II description · CC0")
    #expect(english["dtc_source_vehicle_observed"] != nil)
    #expect(english["dtc_source_unverified"] != nil)
  }
