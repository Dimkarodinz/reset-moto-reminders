...
final class DecoderAndBuilderTests: XCTestCase {
  ...
  func testDtcCountAndDetailsDecodeConfirmedRecord() throws {
    ...
    let decoder = DTCDecoder(
      descriptions: profile.dtcDescriptions,
      descriptionProvenance: profile.dtcDescriptionProvenance)
    ...
    XCTAssertEqual(
      ...
      dtc.message)
    XCTAssertEqual(.vehicleObserved, dtc.descriptionProvenance)

    let generic = try XCTUnwrap(decoder.decodeDetails("59020C00300008").first)
    XCTAssertEqual("P0030-00", generic.code)
    XCTAssertEqual(.openDataGeneric, generic.descriptionProvenance)
  }
  ...
  func testUnknownValidDtcStillGetsReadableFallback() throws {
    ...
    XCTAssertEqual(.unverified, dtc.descriptionProvenance)
  }
