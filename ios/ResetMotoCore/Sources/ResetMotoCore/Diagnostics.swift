...
public struct DiagnosticTroubleCode: Equatable, Identifiable, Sendable {
  ...
  public let descriptionProvenance: DTCDescriptionProvenance
  ...
  public init(
    ...
    confirmed: Bool,
    message: String,
    descriptionProvenance: DTCDescriptionProvenance = .unverified
  ) {
    self.code = code
    ...
    self.descriptionProvenance = descriptionProvenance
  }
}

public struct DTCDecoder: Sendable {
  private let descriptions: [String: String]
  private let descriptionProvenance: [String: DTCDescriptionProvenance]

  public init(
    descriptions: [String: String],
    descriptionProvenance: [String: DTCDescriptionProvenance] = [:]
  ) {
  ...
  public func decodeDetails(_ payload: String) throws -> [DiagnosticTroubleCode] {
    ...
    return (0..<count).map { index in
      ...
      let baseCode = String(code.prefix(5))
      let matchedKey = descriptions[code] != nil ? code : descriptions[baseCode] != nil ? baseCode : nil
      ...
      return DiagnosticTroubleCode(
        ...
        message: descriptions[code] ?? descriptions[baseCode] ?? fallback(code),
        descriptionProvenance: matchedKey.flatMap { descriptionProvenance[$0] } ?? .unverified
      )
    }
  }
