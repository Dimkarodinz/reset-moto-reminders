...
| [`../ecu-maps/triumph.motorcycleprofiles.yaml`](../ecu-maps/triumph.motorcycleprofiles.yaml), [`../ecu-maps/README.md`](../ecu-maps/README.md) | Shared motorcycle catalogue, capability status and family composition |
| [`../ecu-maps/triumph-original-tft.instrumentfamily.yaml`](../ecu-maps/triumph-original-tft.instrumentfamily.yaml), [`../ecu-maps/triumph-updated-tft.instrumentfamily.yaml`](../ecu-maps/triumph-updated-tft.instrumentfamily.yaml), [`../ecu-maps/triumph-hybrid-display.instrumentfamily.yaml`](../ecu-maps/triumph-hybrid-display.instrumentfamily.yaml), [`../ecu-maps/triumph-adaptive-combined.instrumentfamily.yaml`](../ecu-maps/triumph-adaptive-combined.instrumentfamily.yaml) | Executable original, combined and live-shape-detected service-reminder families |
| [`../dtc-maps/triumph-tiger-900-gt-pro-2021.en.dtcmap.yaml`](../dtc-maps/triumph-tiger-900-gt-pro-2021.en.dtcmap.yaml) | Shared user-facing DTC dictionary with observed and CC0 generic entries kept distinct |
...
- Version `0.4.0` (`build 9`) supports English, German, Spanish, French and Ukrainian, with English as the default/fallback. This covers UI copy, Bluetooth/diagnostic failures, DTC descriptions and their provenance labels. Locale selection must never change commands, units or compatibility gates.
...
- The disconnected UI exposes the shared Triumph profile catalogue. The validated Tiger 900 GT Pro remains the default; every additional profile is visibly experimental and carries capability-specific dashboard, DTC and service-reset behavior.
- The original split service path requires live `043`. Combined paths perform session/security and read-only precursor requests, select only their mapped static format or an exact 12-byte/5-byte `A000` live shape, then verify the same resolved format after one write sequence. Unknown shapes stop before `2EA000`; no alternate format is retried.
- DTC clear retains the Beta label and explicit destructive confirmation. Experimental families use their mapped direct strategy only after a positive engine identity response; the validated Tiger retains its security-access strategy.
- Malformed replies after a state-changing request produce a needs-verification/partial outcome. BLE timeout, disconnect and background interruption after a write remain ambiguous and force disconnect.
