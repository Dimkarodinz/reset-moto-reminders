...
- Version `0.3.0` (`build 8`) supports English, German, Spanish, French and Ukrainian, with English as the default/fallback. This covers UI copy, Bluetooth/diagnostic failures and known or generic DTC descriptions. Locale selection must never change commands, units or compatibility gates.
...
- The disconnected UI exposes the shared 10-entry Triumph catalogue. The validated Tiger 900 GT Pro remains the default; every additional profile is visibly experimental and carries capability-specific dashboard, DTC and service-reset behavior.
