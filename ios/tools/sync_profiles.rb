...
descriptions = dtc.fetch("generic_entries").dup
description_provenance = dtc.fetch("generic_entries").keys.to_h do |code|
  [code, "open_data_generic"]
end
dtc.fetch("entries").each { |code, entry| descriptions[code] = entry.fetch("message") }
dtc.fetch("entries").each do |code, entry|
  description_provenance[code] = entry.fetch("message_status")
end
...
%w[de es fr uk].each do |locale|
  ...
  localized = descriptions.merge(translation.fetch("generic_messages"))
...
profile = {
  ...
  "dtcDescriptionProvenance" => description_provenance.sort.to_h,
}
